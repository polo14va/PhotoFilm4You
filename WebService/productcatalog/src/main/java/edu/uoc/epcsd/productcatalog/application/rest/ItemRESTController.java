...
public class ItemRESTController {
    ...
    @GetMapping("/itemsByProduct/{productId}")
    @ResponseStatus(HttpStatus.OK)
    public ResponseEntity<Integer> getItemsByProduct(@PathVariable(name= "productId") @NotBlank int productId) {

        return ResponseEntity.ok().body((int) itemService.findByProductId((long) productId).stream().
                    filter(s -> s.getStatus() == ItemStatus.OPERATIONAL).count());
    }


    @PatchMapping("/{serialNumber}")
    public ResponseEntity<Item> setOperational(@PathVariable(name = "serialNumber") @NotBlank String serialNumber, @RequestBody @NotNull boolean operational) {

        try {
            return ResponseEntity.ok().body(itemService.setOperational(serialNumber, operational));
        } catch (IllegalArgumentException e) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "The specified item serial number '" + serialNumber + "' do not exist.");
        }
    }
